...
**Credentials live in SSM Parameter Store, not Terraform variables.** `aws_ssm_parameter.security_username`/`security_password_hash` (`SecureString`, on the free default `alias/aws/ssm` key - a customer-managed KMS key would cost ~$1/mo for no benefit here) hold the real values; `aws_lambda_function.app`'s environment reads `aws_ssm_parameter.*.value`, which MicroProfile Config then picks up as `TURBOORDERS_SECURITY_USERNAME`/`TURBOORDERS_SECURITY_PASSWORD_HASH`. Both parameters have `lifecycle { ignore_changes = [value] }` - Terraform seeds them once (from `var.security_username`/`security_password_hash`, both defaulted - `turboorders` and `null` respectively, `null` coalescing to an obviously-invalid placeholder hash) and never again overwrites a value someone sets by hand. **To rotate the actual credential: edit the parameter's value in the AWS console (or CLI), then run `terraform apply`/`scripts/deploy.sh` with no `-var`/`TF_VAR_*` flags at all** - it reads the current SSM value and pushes it into the Lambda's environment. Don't move these back to plain Terraform variables read directly into `environment.variables` - that's what made every deploy require typing the password.

The Lambda execution role does **not** have `ssm:GetParameter` - it doesn't need it, since the value is injected into the environment at `terraform apply` time, not fetched by the running app. If a future requirement needs credential rotation to take effect without a redeploy, that would mean switching to a runtime fetch (app calls SSM directly, `platform/security/` needs the AWS SDK + an IAM grant) instead of this apply-time injection - a bigger change, not just an IAM tweak.
